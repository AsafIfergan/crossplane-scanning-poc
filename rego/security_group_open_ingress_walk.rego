...
WizPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("%sspec.%s.ingress[%d].ipRanges[%d].cidrIp", [cp_lib.getPath(path), section, rule_index, cidr_index]),
...
WizPolicy[result] {
	...
	result := {
		...
		"resourceName": value.metadata.name,
		"searchKey": sprintf("%sspec.%s.cidrIpv4", [cp_lib.getPath(path), section]),
		"issueType": "IncorrectValue",
